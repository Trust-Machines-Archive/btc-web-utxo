...
const fromAddressPathInput = (document.getElementById("from-address-path"));
const utxoCountInput = document.getElementById("utxo-count");
...
function load_utxos() {
    ...
    return fetch(url)
        ...
        if (response.error) {
            ...
            let msg = everyUTXO.length + " UTXOs loaded . total BTC: " + value / Math.pow(10, 8);
            document.getElementById("from-address-detail").textContent = msg;
            // load individual UTXOs
            return Promise.all(everyUTXO.map((tx, idx) => {
                return {
                    meta: tx,
                };
            }));
...
//# sourceMappingURL=transacter.js.map